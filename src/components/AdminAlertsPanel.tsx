...
export const AdminAlertsPanel: React.FC<AdminAlertsPanelProps> = ({
  ...
  return (
    <div className="space-y-3 animate-fade-in">
      ...
      <div className="bg-gradient-to-r from-amber-500/10 via-amber-500/5 to-emerald-500/10 border border-amber-300/60 rounded-xl p-3 sm:p-4 shadow-xs">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div className="flex items-start sm:items-center gap-2.5">
            <div className="p-2 bg-amber-500 text-slate-950 rounded-lg shadow-sm shrink-0">
              <BellRing className="w-5 h-5 animate-bounce" />
            </div>
            <div>
              <div className="flex items-center gap-1.5">
                <span className="text-[9px] font-black uppercase tracking-widest text-amber-800 bg-amber-200/70 px-1.5 py-0.5 rounded">
                  Monitoreo en Tiempo Real
                </span>
                {unreadCount > 0 && (
                  <span className="text-[10px] font-bold text-rose-700 bg-rose-100 px-1.5 py-0.5 rounded-full border border-rose-200">
          ...
          <div className="flex flex-wrap items-center gap-1.5">
            ...
            >
              <Volume2 className={`w-3.5 h-3.5 ${testedSound ? 'animate-spin' : 'text-amber-600'}`} />
            ...
            {unreadCount > 0 && (
              <button
                ...
                className="inline-flex items-center gap-1 px-3 py-1.5 text-xs font-bold rounded-lg bg-slate-900 hover:bg-slate-800 text-white transition-all shadow-xs cursor-pointer"
              >
      ...
      {filteredAlerts.length === 0 ? (
        <div className="bg-white border border-slate-200 rounded-xl p-8 text-center">
          ...
          <h3 className="text-xs font-bold text-slate-800">No hay alertas recientes</h3>
              ...
              >
                <div className="flex items-start gap-2.5">
                  ...
                  <div className="space-y-0.5">
                    <div className="flex flex-wrap items-center gap-1.5">
                      <span className="px-2 py-0.5 rounded text-[11px] font-black bg-slate-900 text-white tracking-wide">
                        {alert.routeId}
                      </span>
                      {alert.voucherNumber && (
                        <span className="font-mono text-[11px] font-bold text-emerald-700 bg-emerald-100/70 px-1.5 py-0.5 rounded">
                          {alert.voucherNumber}
                        </span>
                      )}
                      {!alert.read && (
                        <span className="text-[9px] font-black text-rose-700 bg-rose-100 px-1.5 py-0.5 rounded-full uppercase tracking-wider">
                          Nueva Alerta
                        </span>
                      )}
                ...
                <div className="flex items-center gap-1.5 self-end sm:self-center shrink-0">
                  {matchedClaim && (
                    <button
                      ...
                      className="inline-flex items-center gap-1 px-2.5 py-1 text-xs font-bold bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg shadow-xs transition-colors cursor-pointer"
                    >
                  ...
                  {!alert.read && (
                    <button
                      ...
                      className="p-1 text-xs font-semibold text-slate-600 hover:text-slate-900 hover:bg-slate-100 rounded-lg transition-colors cursor-pointer"
