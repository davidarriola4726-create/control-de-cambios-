          ...
          {ALL_ROUTES.map((route) => {
            ...
            return (
              <button
                ...
                className="bg-white border border-slate-200 hover:border-emerald-500 hover:shadow-md rounded-2xl p-5 text-left transition-all group cursor-pointer flex flex-col justify-between"
              >
                <div>
                  <div className="flex items-center justify-between mb-3">
                    <div className="w-10 h-10 rounded-xl bg-emerald-50 group-hover:bg-emerald-600 group-hover:text-white text-emerald-700 flex items-center justify-center transition-colors">
                      <FolderOpen className="w-5 h-5" />
                    </div>
                    <span className="text-xs font-mono font-black px-2 py-0.5 rounded-lg bg-slate-100 text-slate-800 group-hover:bg-emerald-100 group-hover:text-emerald-900 transition-colors">
                      {route}
                    </span>
                  </div>
                  ...
                  <p className="text-[11px] text-slate-400 mt-0.5">Expediente Oficial de Cambios</p>
                </div>

                <div className="mt-4 pt-3 border-t border-slate-100 flex items-center justify-between text-xs">
                  <div>
                    <strong className="text-sm font-black text-slate-900 block leading-none">
                      {routeClaims.length}
                    </strong>
                    <span className="text-[10px] text-slate-400">Reclamos</span>
                  </div>
                  <div className="text-right">
                    <strong className="text-sm font-black text-emerald-700 block leading-none">
                      {totalQuantity}
                    </strong>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-5 sm:p-6 shadow-xs">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-start sm:items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                <span className="px-2.5 py-0.5 rounded-lg text-xs font-black bg-slate-900 text-white font-mono">
                  {selectedRoute}
                </span>
                {isRouteUser && (
                  <span className="text-[10px] font-bold text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded-full border border-emerald-200 flex items-center gap-1">
                    <Lock className="w-3 h-3" /> Su Carpeta Exclusiva
                  </span>
                )}
              </div>
              ...
              <p className="text-xs text-slate-500">
                Historial completo de cambios tramitados con filtros por fecha, producto y motivo.
              </p>
            </div>
          </div>

          <div className="no-print flex items-center gap-2 self-end sm:self-center">
            <button
              type="button"
            ...
            {clearFolderAction && (currentUser.role === 'ADMIN' || selectedRoute === currentUser.routeId) && filteredClaims.length > 0 && (
              ...
              >
                ...
                <span className="hidden sm:inline">Vaciar</span> Carpeta
              </button>
            )}
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-6 pt-5 border-t border-slate-100 text-xs">
          ...
          <div className="bg-slate-50 p-3 rounded-xl border border-slate-100">
            ...
            <strong className="text-lg sm:text-xl font-black text-emerald-700">
      ...
      <div className="no-print bg-white rounded-2xl border border-slate-200 p-4 shadow-xs space-y-3">
        <div className="flex items-center justify-between">
          <span className="text-xs font-black uppercase tracking-wider text-slate-700 flex items-center gap-1.5">
            ...
            Filtros de Búsqueda del Expediente
          </span>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 text-xs">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-8 pr-2.5 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs font-medium focus:ring-2 focus:ring-emerald-500 focus:bg-white"
            />
          </div>
          ...
          <div>
            <select
              ...
              className="w-full px-2.5 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs font-medium focus:ring-2 focus:ring-emerald-500 focus:bg-white"
            >
          ...
          <div className="flex items-center gap-1.5">
            <input
              ...
              className="w-1/2 px-2 py-2 bg-slate-50 border border-slate-200 rounded-xl text-[11px] font-medium focus:ring-2 focus:ring-emerald-500"
              title="Fecha inicial"
            />
      ...
      <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-xs">
        <div className="px-5 py-3.5 border-b border-slate-200 flex items-center justify-between">
          <span className="text-xs font-bold text-slate-700">
            {filteredClaims.length} {filteredClaims.length === 1 ? 'registro encontrado' : 'registros encontrados'}
          </span>
        </div>
        ...
        {filteredClaims.length === 0 ? (
          <div className="p-12 text-center">
            ...
            <h4 className="text-sm font-bold text-slate-800">No hay reclamos registrados con estos filtros</h4>
            ...
            <table className="w-full text-left text-xs">
              <thead className="bg-slate-50 text-slate-600 font-bold uppercase tracking-wider border-b border-slate-200 text-[10px]">
                <tr>
                  ...
                  <th className="py-3 px-4 text-center">Firmas</th>
                ...
                {filteredClaims.map((claim) => (
                  <tr key={claim.id} className="hover:bg-slate-50/80 transition-colors">
                    ...
                    <td className="py-3.5 px-4">
                      ...
                      <span className="font-mono text-[11px] text-slate-500">Factura: {claim.invoiceNumber}</span>
                    </td>
                    <td className="py-3.5 px-4">
                      <span className="font-semibold text-slate-900 block">{claim.productName}</span>
                      {claim.reasonDetails && (
                        <span className="text-[11px] text-slate-500 truncate block max-w-xs" title={claim.reasonDetails}>
                          {claim.reasonDetails}
                        </span>
                      )}
                    </td>
                      ...
                      <div className="flex items-center justify-end gap-1.5">
                        ...
                        {onDeleteClaim && (currentUser.role === 'ADMIN' || claim.routeId === currentUser.routeId) && (
                          <button
                            ...
                            className="p-1.5 bg-rose-50 hover:bg-rose-100 text-rose-600 hover:text-rose-800 rounded-lg border border-rose-200 transition-colors cursor-pointer"
